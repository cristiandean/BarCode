...
class BarCodeComponent extends Component
{
    ...
    )
    {
        ...
        $p_w2n = $this->checkWideToNarrow($p_barcodeType, $p_w2n);
        $p_charGap = $this->checkCharGap($p_barcodeType, $p_charGap);
        $p_check = $this->checkCheckDigit($p_barcodeType, $p_check);
        $quietZone = 10 * $p_xDim; // safe white zone before and after the barcode

        if ($p_check) {
            $textCheck = $this->getCheckDigit($p_barcodeType, $p_origText);
        } else {
            $textCheck = $p_origText;
        }
        $text2bar = $this->getBarcodeText($p_barcodeType, $textCheck); // format text
        $charCount = $this->getCharCount($p_barcodeType, $text2bar); // number of symbols

        // image height & width
        $imgWidth = $this->getBarcodeLength($p_barcodeType, $text2bar, $p_xDim, $p_w2n, $quietZone, $p_charGap);
        ...
        for ($j = 0; $j < $charCount; $j++) { // traverse string
            $currChar = $this->getSpec($p_barcodeType, $text2bar, $j); // get symbol spec.
        ...
        $functionName = $this->barcodeImgFunction($p_imgType); // get php image output function
        if ($p_toFile) {
            $fileExt = $this->barcodeFileExt($p_imgType); // get file extension
            ...
            $headerContent = $this->barcodeHeaderContent($p_imgType); // get header type
    ...
    private function checkWideToNarrow($i, $wtn)
    {
        switch ($i) {
            case BC_TYPE_CODE39:
            case BC_TYPE_STD25:
    ...
    private function checkCharGap($i, $ig)
    {
        switch ($i) {
            case BC_TYPE_CODE39:
            case BC_TYPE_ROYMAIL4:
    ...
    private function checkCheckDigit($type, $ck)
    {
        switch ($type) {
            case BC_TYPE_CODE93:
            case BC_TYPE_ROYMAIL4:
    ...
    private function getCheckDigit($type, $txt)
    {
        $stLen = strlen($txt);
        switch ($type) {
            case BC_TYPE_CODE39: // modulo 43
                $ck = 0;
                for ($i = 0; $i < $stLen; $i++) {
                    $ck = $ck + $this->code393char2value(ord($txt[$i]));
                }
                return $txt . $this->code393value2char($ck % 43);
                break;
            case BC_TYPE_CODE93: // C y K check digits. Modulo 47
                $ck = 0;
                for ($i = 0; $i < $stLen; $i++) {
                    $ck = $ck + $this->code393char2value(ord($txt[$stLen - $i - 1])) * ($i % 20 + 1);
                }
                $txt1 = $txt . $this->code393value2char($ck % 47);
                $stLen++;
                $ck = 0;
                for ($i = 0; $i < $stLen; $i++) {
                    $ck = $ck + $this->code393char2value(ord($txt1[$stLen - $i - 1])) * ($i % 15 + 1);
                }
                return $txt1 . $this->code393value2char($ck % 47);
                break;
            case BC_TYPE_STD25:
            ...
            case BC_TYPE_ROYMAIL4:
                ...
                $uh = 0;
                for ($i = 0; $i < $stLen; $i++) {
                    $chrSpec = strrev($this->getSpec($type, $txt, $i));
                    $tlh = 0;
                    $tuh = 0;
                    for ($j = 0; $j < 4; $j++) {
                ...
                return $txt . $this->roymail4value2char($uh % 6, $lh % 6);
                break;
            case BC_TYPE_POSTNET:
                $ck = 0;
                for ($i = 0; $i < $stLen; $i++) {
    ...
    private function code393char2value($i)
    {
        /*
    ...
    private function code393value2char($i)
    {
        /*
            Code 93 specification for symbols (index represented by ascii codes)
            Used for check digit calculation.
    ...
    private function getSpec($type, $txt, $i)
    {
        switch ($type) {
            case BC_TYPE_CODE39:
                return $this->code39spec(ord($txt[$i]));
                break;
            case BC_TYPE_STD25:
            case BC_TYPE_INTER25:
                return $this->interleaveChars(ord($txt[$i * 2]), ord($txt[$i * 2 + 1]));
                break;
            case BC_TYPE_CODE93:
                return $this->code93spec(ord($txt[$i]));
                break;
            case BC_TYPE_ROYMAIL4:
                return $this->roymail4spec(ord($txt[$i]));
                break;
            case BC_TYPE_POSTNET:
                return $this->postnetspec(ord($txt[$i]));
                break;
    ...
    private function roymail4spec($i)
    {
        /*
    ...
    private function postnetspec($i)
    {
        /*
            Royal Mail 4-State specification for symbols (index represented by ascii codes).
            Some special signs (*) are introduced to manage special control chars.
    ...
    private function roymail4value2char($row, $col)
    {
        /*
    ...
    private function getCharCount($i, $txt)
    {
        switch ($i) {
            case BC_TYPE_CODE39:
            case BC_TYPE_CODE93:
    ...
    private function getBarcodeLength($i, $txt, $xd, $wtn, $qz, $ig)
    {
        switch ($i) {
            case BC_TYPE_CODE39:
    ...
    private function barcodeImgFunction($i)
    {
        switch ($i) {
            case BC_IMG_TYPE_PNG:
                return "imagepng";
                break;
            case BC_IMG_TYPE_JPG:
                return "imagejpeg";
                break;
            case BC_IMG_TYPE_GIF:
                return "imagegif";
                break;
            case BC_IMG_TYPE_WBMP:
                return "imagewbmp";
                break;
            default:
                return "imagepng";
    ...
    private function barcodeFileExt($i)
    {
        switch ($i) {
            case BC_IMG_TYPE_PNG:
                return "png";
...
*/
    ...
    private function barcodeHeaderContent($i)
    {
        switch ($i) {
            case BC_IMG_TYPE_PNG:
                return "image/png";
    ...
    private function safeStr($i, $auxStr)
    { // check whether a string has only valid chars or not
        switch ($i) {
            case BC_TYPE_CODE39:
            case BC_TYPE_CODE93:
